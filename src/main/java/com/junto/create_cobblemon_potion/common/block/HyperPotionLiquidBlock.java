...
public class HyperPotionLiquidBlock extends AbstractEffectLiquidBlock {


    public static final int INITIAL_STRENGTH = 30; // 好伤药假设初始强度 2
    ...
    private static final Supplier<net.minecraft.world.level.block.state.BlockState> DEGRADED_STATE =
            () -> BlockRegistry.SUPER_POTION_BLOCK.get().defaultBlockState().setValue(STRENGTH, SuperPotionLiquidBlock.INITIAL_STRENGTH); // Degrade to Potion Block at its initial strength


    public HyperPotionLiquidBlock(Supplier<? extends FlowingFluid> pFluidSupplier, Properties pProperties) {
...
//        }
        if (strength > 0){
            entity.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 60, 2, false, true, true));
        }
